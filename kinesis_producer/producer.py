...
class FirehoseProducer(object):
    """A Firehose client that publishes records to a Firehose stream."""

    def __init__(self, config):
        log.debug('Starting FirehoseProducer')
        self.config = config
        self._queue = queue.Queue()
        self._closed = False
    ...
    def send(self, record):
        ...
        Don't block. Record must be bytes type.
        """
        assert not self._closed, "FirehoseProducer closed but called anyway"
    ...
    def close(self):
        if self._closed:
            return
        log.debug('Closing FirehoseProducer')
        self._sender.close()
        self._closed = True

    def join(self):
        self.close()
        log.debug('Joining FirehoseProducer')
        self._queue.join()
        log.debug('FirehoseProducer record queue was joined')
        self._sender.join()